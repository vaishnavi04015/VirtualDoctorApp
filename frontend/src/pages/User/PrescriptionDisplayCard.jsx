        ...
        <div>
            ...
            />
            {
                ...
                searchData.map((temp, index) => (
                    <div key={index} className="mb-4 p-4 border border-gray-300 rounded w-[65%] m-8" onClick={()=>handleLink(temp)}>
                        <div className="flex justify-between mb-2">
                            <div>
                                <h2 className="text-xl font-bold">{token ? temp.doctorName : temp.name}</h2>
                                <h2 className="text-gray-600">{token ? temp.doctorEmail : temp.email}</h2>
                            </div>
                            <div>
                                <h3 className="text-gray-600">{temp.dnt.date}</h3>
                                <h3 className="text-gray-600">{temp.dnt.time}</h3>
                            </div>
                        </div>
                    </div>
                ))
